...
int main(void) {
		...
		FILE *fp = fopen("A_data.dat", "rb");
		if (fp != NULL) {
				fread(&count, 4, 1, fp);
				for (i=0; i<count; i++) {
						fread(title[i], 21, 1, fp);
						fread(genre[i], 9, 1, fp);
						fread(&story_score[i], 4, 1, fp);
						fread(&music_score[i], 4, 1, fp);
						fread(&casting_score[i], 4, 1, fp);
						fread(story[i], 101, 1, fp);
						fread(review[i], 1001, 1, fp);
				}
				for (i=0; i<count; i++) {
		...
		while(1) {
				...
				if(user_choice == 1) {
						...
						if(user_choice == 1) {
								FILE *fp = fopen("A_data.dat", "wb");
								count ++;
								fwrite(&count, 4, 1, fp);
								for (i=0; i<count; i++) {
										fwrite(title[i], 21, 1, fp);
										fwrite(genre[i], 9, 1, fp);
										fwrite(&story_score[i], 4, 1, fp);
										fwrite(&music_score[i], 4, 1, fp);
										fwrite(&casting_score[i], 4, 1, fp);
										fwrite(story[i], 101, 1, fp);
										fwrite(review[i], 1001, 1, fp);
								}
								fclose(fp);
						}
